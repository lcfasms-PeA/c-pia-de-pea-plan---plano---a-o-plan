...
export default function NotificationCenter({ onNotificationRead }: NotificationCenterProps) {
  ...
  const { data: notificationList, refetch } = trpc.notifications.list.useQuery();
  const markAsReadMutation = trpc.notifications.markAsRead.useMutation();

  const notifications = useMemo<Notification[]>(() => {
    return (notificationList || []).map((item: any) => ({
      id: item.id,
      titulo: item.title,
      ...
      tipo:
        item.severity === "Erro"
          ...
          : item.severity === "Aviso"
            ? "aviso"
            : item.severity === "Sucesso"
              ? "sucesso"
              : "info",
      lida: Boolean(item.read),
      dataHora: new Date(item.createdAt),
    }));
  }, [notificationList]);
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            {!notif.lida && (
                              <span className="text-xs font-semibold text-blue-600">Marcar como lida</span>
                            )}
                          </div>
                        </div>
                      </div>
